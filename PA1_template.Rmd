...
`SIMean <- aggregate(steps~interval, nona, mean) ggplot(data=SIMean, aes(x=interval,y=steps)) +               geom_line() + labs(title="Average number of steps taken", x="interval (1 interval = 5 minutes)")`
